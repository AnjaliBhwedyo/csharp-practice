...
/*Write the program for Nepal Telecommunication Corporation that charges the following form its domestic users:
First 100 calls(Compulsory fee)    Rs .150
Next call 100 calls                 Rs.2.00 per Call
Above this                         Rs.2.50 per call*/

Console.WriteLine("Enter the number of Calls:");
double Tcall = Convert.ToDouble(Console.ReadLine());
double Total;
if(Tcall<100)
{
	Console.WriteLine("Total is Rs.150");

}
else if(Tcall<200)
{
	Total = 150 + (2 * (Tcall - 100));
	Console.WriteLine("Total :" + Total);
}
else
{
	Total = 350 + (2.5 * (Tcall - 200));// Substracting cost of 200 calls 
	Console.WriteLine("Total :" + Total);
}
